...
# ============================================
# IG Trading - Positions Tab Navigation
# ============================================
...
# Screenshot path
SCREENSHOT_FILE="/sdcard/ig_positions_$(date +%s).png"

echo "🚀 Starting IG Positions Tab navigation..."
...
# 4. Click on Positions tab
echo "📊 Clicking Positions tab (x=$POSITIONS_TAB_X, y=$POSITIONS_TAB_Y)..."
su -c "input tap $POSITIONS_TAB_X $POSITIONS_TAB_Y" 2>/dev/null || input tap $POSITIONS_TAB_X $POSITIONS_TAB_Y
sleep 2
...
WINDOW_TEXT=$(su -c "dumpsys window windows | grep -i 'positions\|open'" 2>/dev/null || echo "")
if [ -n "$WINDOW_TEXT" ]; then
    echo "📄 Found text: $WINDOW_TEXT"
fi

# Alternative: use uiautomator if available
UI_TEXT=$(su -c "uiautomator dump /sdcard/ui_positions.xml 2>/dev/null && cat /sdcard/ui_positions.xml | grep -oE 'text=\"[^\"]*\"' | head -20" 2>/dev/null || echo "")
if [ -n "$UI_TEXT" ]; then
    echo "📝 UI Elements:"
...
echo ""
echo "✅ IG Positions tab navigation complete!"
echo "📸 Screenshot: $SCREENSHOT_FILE"

# Pull screenshot to local if requested
if [ "$1" == "--pull" ]; then
    LOCAL_FILE="/data/data/com.termux/files/home/ig_positions_$(date +%s).png"
