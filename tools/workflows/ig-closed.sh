...
# ============================================
# IG Trading - Closed Tab Navigation
# ============================================
...
# Screenshot path
SCREENSHOT_FILE="/sdcard/ig_closed_$(date +%s).png"

echo "🚀 Starting IG Closed Tab navigation..."
...
# 4. Click on Closed tab
echo "📜 Clicking Closed tab (x=$CLOSED_TAB_X, y=$CLOSED_TAB_Y)..."
su -c "input tap $CLOSED_TAB_X $CLOSED_TAB_Y" 2>/dev/null || input tap $CLOSED_TAB_X $CLOSED_TAB_Y
sleep 2
...
WINDOW_TEXT=$(su -c "dumpsys window windows | grep -i 'closed\|history'" 2>/dev/null || echo "")
if [ -n "$WINDOW_TEXT" ]; then
    echo "📄 Found text: $WINDOW_TEXT"
fi

# Alternative: use uiautomator if available
UI_TEXT=$(su -c "uiautomator dump /sdcard/ui_closed.xml 2>/dev/null && cat /sdcard/ui_closed.xml | grep -oE 'text=\"[^\"]*\"' | head -20" 2>/dev/null || echo "")
if [ -n "$UI_TEXT" ]; then
    echo "📝 UI Elements:"
...
echo ""
echo "✅ IG Closed tab navigation complete!"
echo "📸 Screenshot: $SCREENSHOT_FILE"

# Pull screenshot to local if requested
if [ "$1" == "--pull" ]; then
    LOCAL_FILE="/data/data/com.termux/files/home/ig_closed_$(date +%s).png"
