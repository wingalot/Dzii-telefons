...
class TPManager:
    ...
    def register_position(self, deal_id, pair, direction, entry, sl, tp_levels, size, epic):
        ...
        self.positions[deal_id] = {
            ...
            'tp3_hit': False,
            'sl_hit': False,
            'sl_moved_to_be': False,  # Break-even after TP1
            'sl_moved_to_tp1': False,  # After TP2
    ...
    def check_tp_sl_conditions(self, deal_id, current_price):
        """Check TP and SL conditions for a position"""
        ...
        if direction == 'BUY':
            tp1_hit = pos['tp1'] and current_price >= pos['tp1']
            tp2_hit = pos['tp2'] and current_price >= pos['tp2']
            tp3_hit = pos['tp3'] and current_price >= pos['tp3']
        else:  # SELL
            tp1_hit = pos['tp1'] and current_price <= pos['tp1']
            tp2_hit = pos['tp2'] and current_price <= pos['tp2']
            tp3_hit = pos['tp3'] and current_price <= pos['tp3']
        ...
        if tp1_hit and not pos['tp1_hit']:
            pos['tp1_hit'] = True
            # Move SL to entry for break-even
            if not pos.get('sl_moved_to_be'):
                pos['sl_moved_to_be'] = True
                pos['current_sl'] = entry
                logger.info(f"🎯 TP1 HIT: {pos['pair']} @ {current_price}")
                logger.info(f"  SL moved to ENTRY ({entry}) - break even!")
            else:
                logger.info(f"🎯 TP1 HIT: {pos['pair']} @ {current_price}")
        ...
        if tp2_hit and not pos['tp2_hit']:
            ...
            logger.info(f"  SL moved to TP1 ({pos['tp1']}) - locking profits!")
            self.state['tp2_hits'] += 1
            self.save_state()
        
        # After TP2, check fallback (50% retracement from peak)
        if pos['tp2_hit'] and not pos['tp3_hit']:
            if direction == 'BUY':
                if pos['tp2_peak_price'] is None or current_price > pos['tp2_peak_price']:
                    pos['tp2_peak_price'] = current_price
                fallback_threshold = pos['tp2_peak_price'] - (pos['tp2_peak_price'] - pos['tp2']) * TP_FALLBACK_PERCENT
                if current_price < fallback_threshold:
                    logger.info(f"📉 FALLBACK: {pos['pair']} fell 50% from peak after TP2")
                    return 'close_fallback'
            else:  # SELL
                if pos['tp2_peak_price'] is None or current_price < pos['tp2_peak_price']:
                    pos['tp2_peak_price'] = current_price
                fallback_threshold = pos['tp2_peak_price'] + (pos['tp2'] - pos['tp2_peak_price']) * TP_FALLBACK_PERCENT
                if current_price > fallback_threshold:
                    logger.info(f"📉 FALLBACK: {pos['pair']} rose 50% from trough after TP2")
                    return 'close_fallback'
